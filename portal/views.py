...
# -----------------------------
# (Optional) Remove these if you only use the API endpoints.
# They were older JSON views without CSRF. Keeping them commented out:
# -----------------------------
# from django.views.decorators.csrf import csrf_exempt
# @csrf_exempt
# def edit_student(request, student_id):
#     redir = _require_login_api(request)
#     if isinstance(redir, JsonResponse) and redir.status_code == 401:
#         return redir
...
#         marks = data.get("marks", student.marks)
#         if _validate_marks(marks) is None:
#             return _json_error('Marks must be 0-100.')
#         student.marks = int(marks)
#         student.save()
#         AuditLog.objects.create(teacher=request.teacher, action='update', details=f'Edited student {student.id}')
...
#     if request.method == "POST":
#         student.delete()
#         AuditLog.objects.create(teacher=request.teacher, action='delete', details=f'Deleted student {student_id}')
